...
gtag('js', new Date());
gtag('config', 'G-WHMQ0RMS8S');

// popup for mobiles incompatibility
let timeout1 = setTimeout(mobilepop_func, 3000);

function mobilepop_func(){
document.getElementById("mobilepop").classList.add('removed');
}
...
function loading_circle_func()
{
  document.getElementById("loading_circle").classList.add('hide');
}

function loading_button_func()
{
  document.getElementById("webcamButton").classList.remove('removed');
}
...
if (getUserMediaSupported())
{
  ...
  loader();
  let timeout1 = setTimeout(loading_button_func, 2000);
  let timeout = setTimeout(loading_circle_func, 2000);
...
function enableCam(event)
{
  ...
  document.getElementById("loading_circle").classList.remove('hide');
  let timeout1 = setTimeout(loading_circle_func, 4500);
    ...
    for (let i = 0; i < predictions.length; i++)
    {
      ...
      if(stopped_after1result == true)
      {
        if (counter_check ==5)
        {
          ...
          model_output_final = hand.indexOf(Math.max(...hand)) + 1; // 1 is rock, 3 is scissor & 2 is paper as in line 120
          machine_output = Math.floor(Math.random()*3) + 1; // + 1 to avoid getting 0 from random
          if(machine_output==1)
          {
            document.getElementById("comp-hand").src="rock.jpg";
          }
          else if(machine_output==3)
          {
